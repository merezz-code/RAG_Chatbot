...
export const edpApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    postFile: builder.mutation<void, File>({
      query: (file) => {
        const formData = new FormData();
        formData.append("File", file); // Assurez-vous que la clé "file" correspond au paramètre attendu par votre API .NET

        return {
          url: API_ENDPOINTS.POST_FILES, // Assurez-vous que cette constante est définie dans votre config
          method: "POST",
          body: formData,
          // Note : fetchBaseQuery détecte automatiquement le FormData 
          // et définit le Content-Type sur "multipart/form-data" avec le bon boundary.
        };
      },
      transformErrorResponse: (error) =>
        transformErrorMessage(error, ERROR_MESSAGES.POST_FILES),
      onQueryStarted: async (_, { dispatch, queryFulfilled }) => {
        await handleOnQueryStarted(
          queryFulfilled,
          dispatch,
          ERROR_MESSAGES.POST_FILES,
        );
      },
      invalidatesTags: ["Files"], // Rafraîchit la liste des fichiers après l'upload
    }),
